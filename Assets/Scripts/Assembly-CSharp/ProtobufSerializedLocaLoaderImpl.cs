...
public class ProtobufSerializedLocaLoaderImpl : ILocaLoader
{
	...
	public Dictionary<string, string> Load(string language)
	{
		...
		{
			TextAsset textAsset = Resources.Load(EditorLocaResourcesPath + GetLocaFilenameFromLanguage(language), typeof(TextAsset)) as TextAsset;
			if (textAsset == null)
			{
		...
		SerializedLocalizedTexts serializedLocalizedTexts = m_binarySerializer.Deserialize<SerializedLocalizedTexts>(bytes);
